...
class TestTranspose:
    ...
    def test_transpose_preserves_note_count_and_bar_count(self):
        score = Score(tracks=[melodic_track(n_bars=3, notes_per_bar=4)], resolution=12)
    ...
    def test_drum_track_not_transposed(self):
        ...
        drum_before = [n.pitch for b in score.tracks[1].bars for n in b.notes]
        out = Transpose(7)(copy.deepcopy(score))
        drum_after = [n.pitch for b in out.tracks[1].bars for n in b.notes]
        assert drum_before == drum_after
        # Melodic shifted
        mel_before = [n.pitch for b in score.tracks[0].bars for n in b.notes]
        mel_after = [n.pitch for b in out.tracks[0].bars for n in b.notes]
        assert all(a == b + 7 for a, b in zip(mel_after, mel_before, strict=False))
...
class TestBarWindow:
    ...
    def test_window_is_contiguous(self):
        ...
        score = Score(tracks=[Track(bars=bars, track_type="melodic")], resolution=12)
        random.seed(7)
...
class TestScoreWindow:
    ...
    def test_select_window_all_tracks_share_same_bar_range(self):
        ...
        out = select_window(score, n_bars=4, n_tracks=2, rng=random.Random(1))
        assert out is not None
    ...
    def test_select_window_returns_none_when_score_too_short(self):
        score = Score(tracks=[melodic_track(n_bars=2)], resolution=12)
        out = select_window(score, n_bars=8, n_tracks=1, rng=random.Random(0))
        assert out is None
    ...
    def test_select_window_deterministic_with_seeded_rng(self):
        ...
        a = select_window(score, n_bars=3, n_tracks=2, rng=random.Random(123))
        b = select_window(score, n_bars=3, n_tracks=2, rng=random.Random(123))
        assert a is not None and b is not None
        assert [n.pitch for tr in a.tracks for bar in tr.bars for n in bar.notes] == [
            n.pitch for tr in b.tracks for bar in tr.bars for n in bar.notes
        ]
    ...
    def test_select_window_does_not_mutate_source(self):
        ...
        _ = select_window(score, n_bars=3, n_tracks=1, rng=random.Random(0))
...
class TestMaskBar:
    def test_mask_bar_marks_some_bars_future(self):
        score = Score(tracks=[melodic_track(n_bars=8)], resolution=12)
        cfg = MaskBarConfig(apply_probability=1.0, mode=MaskMode.RANDOM, bar_fraction=0.5)
        random.seed(0)
    ...
    def test_mask_bar_respects_infill_exclusion(self):
        score = Score(tracks=[melodic_track(n_bars=8)], resolution=12)
        cfg = MaskBarConfig(apply_probability=1.0, mode=MaskMode.RANDOM, bar_fraction=1.0)
        infill = {(0, 3)}
    ...
    def test_mask_bar_probability_statistical(self):
        score = Score(tracks=[melodic_track(n_bars=6)], resolution=12)
        cfg = MaskBarConfig(apply_probability=0.5, mode=MaskMode.RANDOM, bar_fraction=0.5)
        random.seed(0)
        masked_runs = 0
        N = 200
        for _ in range(N):
...
class TestInstrumentSwap:
    ...
    def test_instrument_swap_preserves_note_count_and_track_count(self):
        score = Score(tracks=[melodic_track(n_bars=2), melodic_track(n_bars=2)], resolution=12)
...
class TestTrackPermutation:
    def test_track_permutation_preserves_track_count(self):
        score = Score(
            tracks=[
                melodic_track(n_bars=2, base_pitch=60),
    ...
    def test_track_permutation_preserves_track_contents_set(self):
        score = Score(
            tracks=[
                melodic_track(n_bars=1, base_pitch=60),
                melodic_track(n_bars=1, base_pitch=70),
                melodic_track(n_bars=1, base_pitch=80),
            ],
            resolution=12,
        )
        first_pitches_before = sorted(t.bars[0].notes[0].pitch for t in score.tracks)
        random.seed(0)
        out = TrackPermutation()(copy.deepcopy(score))
        first_pitches_after = sorted(t.bars[0].notes[0].pitch for t in out.tracks)
        assert first_pitches_before == first_pitches_after

    def test_track_permutation_is_seedable(self):
        tracks = [melodic_track(n_bars=1, base_pitch=60 + 5 * i) for i in range(4)]
    ...
    def test_track_permutation_changes_order_for_some_seed(self):
        tracks = [melodic_track(n_bars=1, base_pitch=60 + 5 * i) for i in range(6)]
        score = Score(tracks=tracks, resolution=12)
...
class TestAugmentationPipeline:
    ...
    def test_pipeline_composes_transforms_in_order(self):
        ...
        assert _all_pitches(out) == [max(0, min(127, p + 5)) for p in before_pitches]
        assert _all_velocities(out) == [max(1, min(127, int(v * 0.5))) for v in before_vels]

    def test_pipeline_order_matters(self):
        # +20 then -20: 120 -> 127 (clamp) -> 107
        # -20 then +20: 120 ->  100        -> 120
        score = Score(
            tracks=[Track(bars=[make_bar([make_note(pitch=120, vel=80)])], track_type="melodic")],
            resolution=12,
        )
        out_a = AugmentationPipeline([Transpose(20), Transpose(-20)])(copy.deepcopy(score))
        out_b = AugmentationPipeline([Transpose(-20), Transpose(20)])(copy.deepcopy(score))
    ...
    def test_default_training_pipeline_constructs_and_runs(self):
        ...
        score = Score(tracks=[melodic_track(n_bars=2), drum_track(n_bars=2)], resolution=12)
        random.seed(0)
        out = pipeline(score)

...
# --------------------------------------------------------------------------- #
def _ar_request(
    track_id: int = 0, bars=None, *, mask_mode: str = "token", model_dim: int = 4
) -> GenerationRequest:
...
def test_mask_mode_token_without_vocab_support_raises(
    ...
    has_mask_bar = any(d.get("type") == "MaskBar" for d in (parsed.get("token_domains") or []))
...
def test_mask_mode_token_with_vocab_support_passes_without_masked_bars(
    ...
    has_mask_bar = any(d.get("type") == "MaskBar" for d in (parsed.get("token_domains") or []))
    assert has_mask_bar, (
        "supports_mask_bar_token=True must include a MaskBar token domain in the serialised config"
    )

"""Shared utilities for comparison tests between midigpt_legacy and midigpt.

Both packages are installed side-by-side:
  - `midigpt_legacy` (original C++/pybind11 module, source in legacy/)
  - `midigpt`        (refactored package, source at repo root)

Install both before running:
  pip install -e .            # new midigpt
  pip install -e ./legacy     # midigpt_legacy (builds old C++ extension)
"""

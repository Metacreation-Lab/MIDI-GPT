"""Pure-PyTorch GPT-2 mirror of the TorchScript checkpoint layout.

State-dict layout matches HuggingFace `GPT2LMHeadModel` exactly:
...
Attention uses `F.scaled_dot_product_attention` — the causal mask is generated
by the kernel, so the legacy `attn.bias` / `attn.masked_bias` buffers are
dropped (stripped during checkpoint loading for backward compat).

The packed checkpoint format produced by `scripts/pack_checkpoint.py` is:
    {
      "format_version": 1,
      "config":         {...},   # GPT-2 arch config
      "encoder_config": {...},   # tokenizer/encoder config (JSON dict)
      "state_dict":     {...},
    }
and is loaded via `from_pretrained(path, device=...)` with auto-config. The
loaded model exposes `model.encoder_config` (dict) for downstream tokenizers.
"""

...
class MidiGPTServer:
    ...
    def __init__(self, engine: InferenceEngine, listen_port: int, max_attempts: int) -> None:
        self._engine = engine
        self._ckpt = "unknown" # legacy
        ...
        log.info("MidiGPTServer initialised — checkpoint: %s", self._ckpt)
    ...
    def _maybe_trigger_generation(self, last_ts_num: int, last_ts_den: int) -> None:
        ...
        try:
            score, req, window_start = piece.to_score_and_request(
                playhead, target_bar, num_anticipation, params["mask_gap"], params
            )
    ...
    def _gen_worker(self) -> None:
        ...
        while True:
            ...
            window_start    = req.get("window_start", 0)
            ...
            _res: list = [None]   # [res_score]
            _exc: list = [None]   # [exception]

            def _infer():
                try:
                    session = self._engine.session(req["score"], req["req"])
                    _res[0] = session.run()
                except Exception as exc:  # noqa: BLE001
                    _exc[0] = exc
            ...
            generated = self._piece.merge_generated(
                res_score, target_bar, num_anticipation, window_start
            )

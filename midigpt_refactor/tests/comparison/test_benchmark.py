...
def test_full_generation_speed(yellow_orig_encoder, yellow_ref_components, capsys):
    ...
    - ref:  midigpt_refactor InferenceEngine, model = GPT2LMHeadModel.from_pretrained
    ...
    from midigpt_refactor.inference.model import GPT2LMHeadModel, resolve_device
    ...
    with capsys.disabled():
        ...
        print(f"orig = midigpt C++ (loads model on each call)  |  ref = midigpt_refactor (model pre-loaded + KV cached)")
